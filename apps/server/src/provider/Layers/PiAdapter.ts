      ...
      withThreadLock(
        ...
        Effect.gen(function* () {
            ...
            yield* acp.handleElicitation((params) =>
              mapAcpCallbackFailure(
                Effect.gen(function* () {
                  ...
                  const mapped = buildPiFormElicitationQuestions(params);
                  if (!mapped.ok) {
                    return yield* new EffectAcpErrors.AcpTransportError({
                      detail: mapped.detail,
                      cause: new Error(mapped.detail),
                    });
                  }
                  ...
                  if (params.mode !== "form") {
                    return yield* new EffectAcpErrors.AcpTransportError({
                      detail: "Pi requested unsupported URL elicitation.",
                      cause: new Error("Pi requested unsupported URL elicitation."),
                    });
                  }
                  return {
                    action: {
                      action: "accept" as const,
                      content: buildPiFormElicitationContent(params, resolved.answers),
                    },
                  };
                }),
              ),
          ...
          const boundModelId = yield* applyPiAcpModelSelection({
            ...
            selections: piModelSelection?.options,
            mapError: (cause) =>
